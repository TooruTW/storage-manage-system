...
const Cost = ({ object }: CostProps) => {
  ...
  useEffect(() => {
    const filteredData = FAKE_SUPPLIER_DATA.filter(
      (item) =>
  ...
  return (
    <ul className="w-full h-full overflow-y-auto flex flex-col gap-2 pb-20">
      {data.map(
        (item) => {
          return (
            <li key={item.supplierName} className="w-full flex gap-2 items-center  rounded-md border-1 border-primary/10 p-2">
